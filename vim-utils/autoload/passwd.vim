...
function! passwd#CopyPasswdToClipboard(passwd_file) abort
	if !executable('gpg')
	...
	return timer_start(g:passwd_sec_on_clipboard*1000, funcref('passwd#ClearSystemClipboard'))
endfunction
...
function! passwd#SelectPasswdFile() abort
	if exists(':Denite')
		...
		" duplicated here 
		return passwd#CopyPasswdToClipboard(g:passwd_store_dir . '/' . passwd_file)
	else
...
function! passwd#ClearSystemClipboard(timer) abort
